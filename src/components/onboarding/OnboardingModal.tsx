...
export default function OnboardingModal() {
  ...
  const BackButton = () => (
    <button onClick={goBack} className="self-start text-[#4A5F7A] hover:text-[#7A8FAD] text-xs flex items-center gap-1 transition-colors">
  ...
  const PrimaryButton = ({ onClick, disabled, children }: { onClick: () => void; disabled?: boolean; children: React.ReactNode }) => (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-full py-3 rounded-xl bg-gradient-to-r from-[#4F8EF7] to-[#6C63FF] text-white font-semibold text-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-2 disabled:opacity-50"
    >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#080B14]/95 backdrop-blur-sm">
          ...
          <div
            ...
            className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
              i === step
                ...
                : i < step
                ? 'bg-[#4F8EF7]/40'
                : 'border border-[#1E2A42] bg-transparent'
            }`}
          />
        ))}
      </div>
      ...
      <div
        key={step}
        className="bg-[#0E1420] border border-[#1E2A42] rounded-2xl p-8 max-w-md w-full mx-4 animate-fade-up"
      >
        {/* Step 0: Welcome */}
        {step === 0 && (
          <div className="flex flex-col items-center text-center gap-6">
            ...
            <div>
              ...
              <p className="text-[#7A8FAD] mt-2 text-sm">
        ...
        {step === 1 && (
          <div className="flex flex-col gap-5">
            ...
            <div className="text-center">
              ...
              <p className="text-[#7A8FAD] mt-1 text-sm">
                A Claude API Key permite análises inteligentes das suas campanhas.
              </p>
            </div>
            <div>
              <label className="text-xs text-[#4A5F7A] mb-1.5 block">Claude API Key</label>
              <input
                type="password"
                value={claudeKey}
                onChange={(e) => { setClaudeKey(e.target.value); setKeyError(''); }}
                placeholder="sk-ant-..."
                className="w-full bg-[#080B14] border border-[#1E2A42] rounded-lg px-4 py-3 text-sm text-[#F0F4FF] placeholder:text-[#4A5F7A]/50 focus:outline-none focus:border-[#4F8EF7] transition-colors"
              />
              {keyError && <p className="text-[#F05252] text-xs mt-1.5">{keyError}</p>}
            </div>
            <a
              href="https://console.anthropic.com"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs text-[#4F8EF7] hover:text-[#4F8EF7] flex items-center gap-1 transition-colors"
            >
        ...
        {step === 2 && (
          <div className="flex flex-col gap-5">
            ...
            <p className="text-[10px] text-[#4A5F7A] text-center">
              Ou cole um token manualmente na aba Config após o onboarding
            </p>
            <button
              onClick={goNext}
              className="text-xs text-[#4A5F7A] hover:text-[#7A8FAD] transition-colors mx-auto"
            >
              Pular por agora
            </button>
          </div>
        ...
        {step === 3 && (
          <div className="flex flex-col items-center text-center gap-6">
            <BackButton />
            <CheckCircle2 className="w-16 h-16 text-[#22D07A] animate-bounce" />
            <div>
