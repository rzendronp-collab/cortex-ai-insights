...
export default function ChatTab() {
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[#E4E7EF] rounded-xl flex flex-col h-[600px] animate-fade-up">
      {/* Context banner */}
      {analysisData && (
        <div className="px-4 py-2 border-b border-[#E4E7EF] bg-[#2563EB]/5 flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : ''}`}>
            {m.role === 'assistant' && (
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#2563EB] to-[#2563EB] flex items-center justify-center flex-shrink-0">
            ...
            <div
              className={`max-w-[75%] px-4 py-3 text-[13px] leading-relaxed ${
                m.role === 'user'
                  ? 'bg-gradient-to-br from-[#2563EB] to-[#2563EB] text-white'
                  : 'bg-[#FFFFFF] border border-[#E4E7EF] text-text-primary'
              }`}
              style={{
            ...
            {m.role === 'user' && (
              <div className="w-8 h-8 rounded-full bg-[#E4E7EF] flex items-center justify-center flex-shrink-0">
        ...
        {loading && (
          <div className="flex gap-3">
            ...
            <div className="bg-[#FFFFFF] border border-[#E4E7EF] rounded-xl px-4 py-3">
        ...
        <div className="px-4 pb-2 flex gap-2 flex-wrap">
          {suggestions.map(s => (
            <button
              ...
              className="text-[11px] px-3 py-1.5 rounded-full border border-[#E4E7EF] text-text-muted hover:text-[#2563EB] hover:border-[#2563EB] transition-all duration-200"
            >
              {s}
            </button>
          ))}
      ...
      {/* Input */}
      <div className="border-t border-[#E4E7EF] p-3 flex gap-2">
        <textarea
          value={input}
          onChange={e => setInput(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(input); } }}
          placeholder="Pergunte sobre suas campanhas..."
          className="flex-1 bg-[#FFFFFF] border border-[#E4E7EF] rounded-lg px-3 py-2 text-sm text-text-primary placeholder:text-text-muted resize-none focus:outline-none focus:border-[#2563EB] min-h-[40px] max-h-[100px] transition-colors duration-200"
          rows={1}
          disabled={loading}
        />
        <Button
          ...
          size="icon"
          className="h-10 w-10 bg-gradient-to-r from-[#2563EB] to-[#2563EB] text-white hover:opacity-90 rounded-lg"
        >
